...
exports.handler = async (event) => {
  ...
  if (!receiver.verified) return json(400, { error: 'This profile is not yet verified.' });
  ...
  try {
    ...
    return json(201, {
      message: `Your enquiry to ${receiverName} was sent successfully.`,
      messageId: msg.id,
    });
  } catch (err) {
    console.error('send-message error:', err);
    return json(500, { error: 'Could not send enquiry. Please try again.' });
  }
};
