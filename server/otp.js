...
function sb(){
 ...
 return {base,key,headers:{'Content-Type':'application/json',apikey:key,Authorization:`Bearer ${key}`,Prefer:'return=representation'}}
}
...
async function sendEmail(to,name,code,subject='Your Floox verification code'){
 ...
 // floox.in is verified in Resend. Keep the sender deterministic so a
 // missing optional RESEND_FROM variable cannot break production OTP.
 const from=process.env.RESEND_FROM || 'no-reply@floox.in';
 if(!key) throw new Error('Email delivery is not configured. RESEND_API_KEY is missing.');
...
async function issue(email,userId,purpose,name){
 const s=sb();
 const code=otp();
 const now=new Date();
...
async function verify(email,code,purpose){
 const s=sb();
 const r=await fetch(`${s.base}/rest/v1/email_otps?email=eq.${encodeURIComponent(email)}&purpose=eq.${purpose}&used_at=is.null&order=created_at.desc&limit=1`,{headers:s.headers});
 const rows=await r.json();
 if(!r.ok||!rows[0]) return {ok:false,error:'No active verification code found.'};
 const row=rows[0];
 if(new Date(row.expires_at)<new Date()) return {ok:false,error:'This code has expired. Please request a new one.'};
 if(row.attempts>=5) return {ok:false,error:'Too many attempts. Please request a new code.'};
 if(hash(code)!==row.code_hash){
   await fetch(`${s.base}/rest/v1/email_otps?id=eq.${encodeURIComponent(row.id)}`,{method:'PATCH',headers:s.headers,body:JSON.stringify({attempts:row.attempts+1})});
   return {ok:false,error:'Invalid verification code.'};
 }
 await fetch(`${s.base}/rest/v1/email_otps?id=eq.${encodeURIComponent(row.id)}`,{method:'PATCH',headers:s.headers,body:JSON.stringify({used_at:new Date().toISOString()})});
