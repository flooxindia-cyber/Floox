...
(() => {
  ...
  function loadScript(file,marker){if(document.querySelector(`script[data-${marker}]`))return;const s=document.createElement('script');s.src=file+'?v=20260818-4';s.async=false;s.dataset[marker]='1';document.head.appendChild(s)}
  ...
  // Follow system: shared by discovery and the fan dashboard.
  if(/floox-search-results\.html$/i.test(location.pathname)||/floox-dashboard-fan\.html$/i.test(location.pathname)){
    const load=()=>loadScript('floox-follows.js','flooxFollows');if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',load,{once:true});else load()
  }
})();
