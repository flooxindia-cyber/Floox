...
(() => {
  ...
  const getGlobalEvents=params=>apiGet('global-events'+(params&&Object.keys(params).length?'?'+new URLSearchParams(params):''),false);
  ...
  function loadScript(file,marker){if(document.querySelector(`script[data-${marker}]`))return;const s=document.createElement('script');s.src=file+'?v=20260821-1';s.async=false;s.dataset[marker]='1';document.head.appendChild(s)}
  if(/floox-dashboard-organiser\.html$/i.test(location.pathname)){const load=()=>{loadScript('organiser-dashboard-live.js','flooxOrganiserLive');loadScript('organiser-messages-live.js','flooxOrganiserMessages');loadScript('floox-marketplace.js','flooxMarketplace')};if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',load,{once:true});else load()}
