...
(() => {
  ...
  function loadScript(file,marker){if(document.querySelector(`script[data-${marker}]`))return;const s=document.createElement('script');s.src=file+'?v=20260818-7';s.async=false;s.dataset[marker]='1';document.head.appendChild(s)}
  ...
  if(/floox-search-results\.html$/i.test(location.pathname)){const load=()=>{loadScript('floox-follows.js','flooxFollows');loadScript('floox-marketplace.js','flooxMarketplace');loadScript('floox-profile-view-quota.js','flooxProfileQuota')};if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',load,{once:true});else load()}
